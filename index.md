---
layout: default
title: Portofolio
---

# 👋 Halo, Saya Muhammad Zenal Arifin

## 🔒 Offensive Security Engineer & Cybersecurity Expert

Selamat datang di portofolio saya! Saya adalah seorang **Offensive Security Engineer** yang passionate dalam mengidentifikasi dan mengeksploitasi kerentanan sistem untuk meningkatkan keamanan organisasi.

---

## 📖 Tentang Saya

Saya adalah seorang **Offensive Security Engineer** dengan pengalaman lebih dari 6 tahun dalam penetration testing, bug bounty hunting, dan vulnerability assessment. Saya adalah founder dari **Security Crash (Secrash)**.

**Fokus utama saya:**
- 🎯 Penetration Testing (Web, Mobile, Desktop, ATM, Infrastructure)
- 🔴 Bug Bounty Hunting dan Vulnerability Research
- 🛡️ Security Training dan Education
- 🔐 CVE Discovery dan Responsible Disclosure

---

## 🛠️ Skills & Technologies

### Penetration Testing Tools
- **Burp Suite**, **OWASP ZAP**, **Nessus**
- **Metasploit**, **Cobalt Strike**, **Empire**
- **Nmap**, **Masscan**, **Zmap**
- **Wireshark**, **tcpdump**, **netcat**

### Programming & Scripting
- **Python** (Penetration Testing Scripts)
- **Bash**, **PowerShell**, **C/C++**
- **JavaScript**, **PHP**, **SQL**

### Operating Systems & Platforms
- **Kali Linux**, **Parrot Security OS**
- **Windows**, **Linux**, **macOS**
- **Active Directory**, **Exchange**
- **Cloud Platforms** (AWS, Azure, GCP)

### Specialized Areas
- **Mobile Pentest** (Android/iOS - Static & Dynamic Analysis)
- **Web Application Security** (OWASP Top 10)
- **Desktop & ATM Penetration Testing**
- **Infrastructure Security Assessment**
- **Vulnerability Assessment** (Nessus, Acunetix, Archerysec)

---

## 🎯 Proyek Unggulan

### 1. Security Crash (Secrash) - Founder & Writer
**Platform:** Educational Security Community (Okt 2021 - Present)

Komunitas pembelajaran cybersecurity untuk publik:
- 📚 Content creation dan tutorial security
- 💬 Platform diskusi cybersecurity
- 🎓 Educational resources dan tips & tricks
- 👥 Community management dan engagement

[🔗 Website](https://secrash.com) | [📄 GitHub](https://github.com/z3n70)

### 2. Bug Bounty Achievements
**Platforms:** HackerOne, Redstorm, Private Programs

Penemuan vulnerability di berbagai platform besar:
- 🏦 **Bank Neo** - Parameter Tampering Loan
- 🏢 **Amazon** - Broken Access Control
- 💰 **Julo** - No Rate Limit Attack
- 🚗 **Moladin** - Broken Access Control
- 📊 **Bibit & Stockbit** - No Rate Limit Bypass via DotTrick

[🔗 GitHub](https://github.com/z3n70)

### 3. CVE Discoveries
**Organizations:** Mozilla, JD.id, Google, Pitch, PhpIPAM

Penemuan vulnerability yang menghasilkan CVE assignments:
- 🔥 **CVE-2023-0676**
- 🔥 **CVE-2023-0677**
- 🛡️ Responsible disclosure process
- 📊 Impact assessment dan remediation

[🔗 CVE Details](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2023-0676) | [📄 Technical Report](./reports/cve-discoveries.pdf)

---

## 📈 Pengalaman

### IT Security Consultant | PT. Punggawa Siber Solusi (2022 - 2025)
- 🎯 Melakukan penetration testing untuk berbagai klien enterprise
- 🚀 Mengembangkan solusi security consulting
- 📊 Vulnerability assessment dan risk analysis
- 🔧 Security training dan education

### IT Security | PT. Weyland Indonesia Perkasa (2020 - 2022)
- 🔍 Awal karir di bidang cybersecurity
- 🤝 Implementasi security controls dan policies
- 🐛 Security monitoring dan incident response
- 📱 Security awareness training

### Freelance Bug Hunter (2018 - Present)
- 🎯 Bug bounty hunting di platform HackerOne, Redstorm
- 🚀 Penemuan vulnerability di platform besar
- 📊 Responsible disclosure dan CVE assignments
- 🔧 Security research dan vulnerability analysis

---

## 🎓 Pendidikan & Sertifikasi

### Pendidikan
- **Bachelor of Informatics Engineering** - STMIK AMIK (2022 - Present)
- **Social Sciences** - SMAN 1 Bojongsoang (2017 - 2020)
- **SMPN 1 Bojongsoang** (2015 - 2017)

### Sertifikasi Cybersecurity
- **OSCP (Offensive Security Certified Professional)** - Offensive Security (Sep 2024)
- **eCPPT (Certified Professional Penetration Tester)** - IneSecurity (Aug 2024)
- **eWPTX (Web Application Penetration Tester eXtreme)** - IneSecurity (Jan 2025)
- **eMAPT (Mobile Application Penetration Tester)** - IneSecurity (Mar 2023)
- **CAP (Certified Appsec Practitioner)** - SecOps (May 2024)
- **CNSP (Certified Network Security Practitioner)** - SecOps (May 2024)
- **C3SA (Certified Cyber Security Analysis)** - Cyberwarfare (Jan 2025)
- **MCRTA (Multi-Cloud Red Team Analyst)** - Cyberwarfare (Feb 2025)
- **CPSA (CREST Practitioner Security Analysis)** - CREST (May 2025)
- **CSFPC (Cyber Security Foundation Professional Certificate)** - CertiProf (Apr 2021)

---

## 🏆 Prestasi & Penghargaan

- 🏅 **CVE Discoveries** - CVE-2023-0676, CVE-2023-0677
- 📝 **Security Trainer**
- 👨‍💻 **Bug Bounty Achievements** - Successful vulnerability discoveries
- 🛡️ **Security Researcher** - Responsible disclosure untuk Mozilla, JD.id, Google, Pitch, PhpIPAM

---

## 📞 Hubungi Saya

Saya selalu terbuka untuk diskusi tentang proyek security, kolaborasi, atau sekedar ngobrol tentang cybersecurity!

- 📧 **Email:** [zenalarifin0x37@gmail.com](mailto:zenalarifin0x37@gmail.com)
- 📱 **Phone:** [+62 8211 7233 669](https://wa.me/6282117233669)
- 🐙 **GitHub:** [github.com/z3n70](https://github.com/z3n70)
- 🌐 **Website:** [Security Crash (Secrash)](https://secrash.com)
- 💼 **LinkedIn:** [Muhammad Zenal Arifin](https://linkedin.com/in/muhammad-zenal-arifin)

---

## 📝 Blog & Artikel

Saya juga aktif menulis artikel tentang cybersecurity dan offensive security di **Security Crash (Secrash)**:

- [🔗 "Mobile Application Penetration Testing Guide"](https://secrash.com/blog/mobile-pentest-guide)
- [🔗 "Web Application Security: OWASP Top 10 Deep Dive"](https://secrash.com/blog/owasp-top10)
- [🔗 "Bug Bounty Hunting: Tips dan Trik untuk Pemula"](https://secrash.com/blog/bug-bounty-tips)
- [🔗 "ATM Penetration Testing: Security Assessment"](https://secrash.com/blog/atm-pentest)
- [🔗 "Infrastructure Security: Network Penetration Testing"](https://secrash.com/blog/infrastructure-pentest)

---

*Terima kasih telah mengunjungi portofolio saya! Jangan ragu untuk menghubungi saya jika ada yang ingin didiskusikan.* 🚀
